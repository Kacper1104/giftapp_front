		...
		return (
			<Card.Text>
				<ListGroup responsive="md" variant="flush">
					<ListGroup.Item>
						Opis: {this.props.gift.gift_description}
					</ListGroup.Item>
					<ListGroup.Item>
						Link:{" "}
						{this.props.gift.gift_link ? (
							<a href={this.props.gift.gift_link}>
								{this.props.gift.gift_link}
							</a>
						) : (
							"-"
					...
					<ListGroup.Item>
						Model:{" "}
						{this.props.gift.gift_model ? this.props.gift.gift_model : "-"}
					</ListGroup.Item>
					<ListGroup.Item>
						Cena (około):{" "}
						{this.props.gift.gift_price
							? this.props.gift.gift_price + " zł"
							: "-"}
					</ListGroup.Item>
				</ListGroup>
			</Card.Text>
			...
			<Card ref={this.cardRef} className={isMobile ? "gift mx-auto" : "gift"}>
				...
				>
						...
						{this.props.isOrganiser ? undefined : this.props.gift.user_res ===
						  1 ? (
							this.props.gift.res_max_contributors > 1 ? (
								<Badge variant="success" className="pull-right">
									Zrzutka{" "}
									{this.props.gift.res_count +
										"/" +
										this.props.gift.res_max_contributors}
								</Badge>
							) : (
								<Badge variant="success" className="pull-right">
									Moje
							...
							this.props.gift.res_count <
							this.props.gift.res_max_contributors ? (
								<Badge variant="warning" className="pull-right">
									Zrzutka{" "}
									{this.props.gift.res_count +
										"/" +
										this.props.gift.res_max_contributors}
								...
								<Badge variant="danger" className="pull-right">
									Zrzutka{" "}
									{this.props.gift.res_count +
										"/" +
										this.props.gift.res_max_contributors}
							...
							<Badge variant="info" className="pull-right">
								Wolny
							</Badge>
						)}
					...
					{this.state.isExpanded ? (
						...
						<Card.Text
							className={this.state.isExpanded ? undefined : "truncate"}
						>
							{this.props.gift.gift_description}
						</Card.Text>
					)}
				</Card.Body>
				<Card.Footer>
					...
					{this.props.isOrganiser ? (
						...
						<Button
							variant="light"
							type="button"
							onClick={this.props.setClickedId}
							className="pull-left"
						>
							{" "}
							Zarezerwuj{" "}
