...
class RegisterUser extends Component {
	...
	register() {
		...
		user.register(this.state.fields["password"]).then((result) => {
			if (result === 201) {
				...
				this.setState({
					showModal: true,
					modalTitle: "Wystąpił nieoczekiwany błąd.",
					modalBody: "Proszę spróbować później."
				});
			}
		});
	}
	...
	render() {
		return (
			<div
				className={
					isMobile ? "body-form login-form" : "body-form login-form text-center"
				}
			>
				<MyModal
						...
						<InputGroup>
							...
							<InputGroup.Append>
								<div
									...
									onClick={() =>
										isMobile
											? (this.passwordInputRef.current.type =
													this.passwordInputRef.current.type === "password"
														? "text"
														: "password")
											: undefined
							...
							<InputGroup.Append>
								<div
									...
									onMouseDownCapture={() =>
										isMobile
											? undefined
											: (this.repeatPasswordInputRef.current.type = "text")
									}
									onMouseLeave={() =>
										isMobile
											? undefined
											: (this.repeatPasswordInputRef.current.type = "password")
									}
									onMouseUpCapture={() =>
										isMobile
											? undefined
									...
									onClick={() =>
										isMobile
											? (this.repeatPasswordInputRef.current.type =
													this.repeatPasswordInputRef.current.type ===
													"password"
														? "text"
														: "password")
